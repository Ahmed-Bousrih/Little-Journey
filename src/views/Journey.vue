...
const lines = computed(() => [
  ...
  `You gave me happiness in the lowest time of my life…`,
...
function calculateTimer(text) {
  const typingTime = (text.length * TYPE_SPEED) / 1000;
  return Math.ceil(typingTime + READING_TIME);
}
...
function nextStep() {
  if (step.value < lines.value.length - 1) {
    step.value++;
    const currentLine = lines.value[step.value];
    secondsLeft.value = calculateTimer(currentLine);
    runTyped(currentLine);
  } else {
...
onMounted(async () => {
  ...
  secondsLeft.value = calculateTimer(firstLine);
  runTyped(firstLine);
  startTimer();
});
